...
namespace Day22
{
    ...
    {
        private static readonly Dictionary<string, int> effects = new Dictionary<string, int>
        {
            { "magic",53 },
            {"drain",73},
            {"shield",113},
        ...
        private static void Round(Player player, Boss boss, int spent, bool part2, List<string> log)
        {
        ...
        private static void ApplyEffects(Player p, Boss b, List<string> _log)
        {
            if (p.Recharge > 0)
            {
                p.Mana += 101;
